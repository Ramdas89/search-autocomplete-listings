...
import SearchAutocompletelist, {
  DEFAULT_INPUT_DEBOUNCE,
  SearchAutocompletelistProps
} from '../src/components/SearchAutocompletelist'
...
describe('<SearchAutocompletelist>', () => {
  ...
  let defaultProps: SearchAutocompletelistProps<Item> = {
    items,
    placeholder: 'Search'
  }
  ...
  it('renders the search box', () => {
    const { queryByPlaceholderText, container } = render(
      <SearchAutocompletelist<Item> {...defaultProps} />
    )
  ...
  it('applies the custom class to the component', () => {
    const { container } = render(
      <SearchAutocompletelist<Item> {...defaultProps} className="search" />
    )
  ...
  it('uses inputSearchString prop', async () => {
    const { queryByPlaceholderText } = render(
      <SearchAutocompletelist<Item> {...defaultProps} inputSearchString="a string" />
    )
  ...
  it('updates the value if inputSearchString prop is updated', async () => {
    ...
    rerender(<SearchAutocompletelist<Item> {...defaultProps} inputSearchString="a new string" />)
  ...
  it('updates results if items change', async () => {
    const { rerender } = render(
      <SearchAutocompletelist<Item> {...defaultProps} onSearch={onSearch} />
    )
    ...
    rerender(
      <SearchAutocompletelist<Item> {...defaultProps} items={newItems} onSearch={onSearch} />
    )
  ...
  it('returns the items on focus if showItemsOnFocus is true', async () => {
    const { queryByPlaceholderText, container } = render(
      <SearchAutocompletelist<Item> {...defaultProps} showItemsOnFocus={true} />
    )
  ...
  it('limits the items on focus if showItemsOnFocus is true to maxResults', async () => {
    const { queryByPlaceholderText, container } = render(
      <earchAutocompletelist<Item> {...defaultProps} showItemsOnFocus={true} maxResults={2} />
    )
  ...
  it('calls onHover when result is hovered', () => {
    ...
    const { container, queryByPlaceholderText } = render(
      <SearchAutocompletelist<Item> {...defaultProps} onHover={onHover} />
    )
  ...
  it('calls onSelect when key navigating down and pressing return', () => {
    ...
    const { queryByPlaceholderText } = render(
      <SearchAutocompletelist<Item> {...defaultProps} onSelect={onSelect} />
    )
  ...
  it('sets the value of the input to the selected item when pressing return', () => {
    const { queryByPlaceholderText } = render(<SearchAutocompletelist<Item> {...defaultProps} />)
  ...
  it('does not display results again after selection if items changes', () => {
    ...
    render(
      <SearchAutocompletelist<Item> {...defaultProps} items={newItems} onSelect={onSelect} />,
      {
        container
      }
    )
  ...
  it('calls onFocus on input focus', () => {
    ...
    const { queryByPlaceholderText } = render(
      <SearchAutocompletelist<Item> {...defaultProps} onFocus={onFocus} />
    )
  ...
  it('sets focus if autoFocus is true', () => {
    const { queryByPlaceholderText } = render(
      <SearchAutocompletelist<Item> {...defaultProps} autoFocus={true} />
    )
  ...
  it("doesn't use debounce if inputDebounce is 0", () => {
    const { queryByPlaceholderText } = render(
      <SearchAutocompletelist<Item> {...defaultProps} onSearch={onSearch} inputDebounce={0} />
    )
  ...
  describe('with items with name property', () => {
    ...
    it('shows nothing if showNoResults is false', () => {
      const { queryByPlaceholderText, container } = render(
        <SearchAutocompletelist<Item> {...defaultProps} showNoResults={false} />
      )
  ...
  describe('with items with custom properties property', () => {
    ...
    it('shows 4 matching items', () => {
      const { queryByPlaceholderText, container } = render(
        <SearchAutocompletelist<ItemWithDescription> {...defaultProps} />
      )
  ...
  describe('showClear', () => {
    ...
    it('displays the clear icon when showClear is true', () => {
      const { queryByPlaceholderText, container } = render(
        <SearchAutocompletelist<Item> {...defaultProps} showClear={true} />
      )
      const inputElement = queryByPlaceholderText(/search/i)
    ...
    it('hides the clear icon when showClear is false', () => {
      const { queryByPlaceholderText, container } = render(
        <SearchAutocompletelist<Item> {...defaultProps} showClear={false} />
      )
      const inputElement = queryByPlaceholderText(/search/i)
    ...
    it('clears the text, sets focus and calls onClear when the clear icon is clicked', () => {
      ...
      const { queryByPlaceholderText, container } = render(
        <SearchAutocompletelist<Item> {...defaultProps} onClear={onClear} onFocus={onFocus} />
      )
      const inputElement = queryByPlaceholderText(/search/i) as HTMLInputElement
  ...
  it('limits number of inputtable characters', () => {
    const onSearch = jest.fn()
    const { queryByPlaceholderText } = render(
      <SearchAutocompletelist<Item> {...defaultProps} maxLength={2} onSearch={onSearch} />
    )
    const inputElement = queryByPlaceholderText(/search/i) as HTMLInputElement
